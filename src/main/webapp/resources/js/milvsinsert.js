	...
	//---------------------------------------------------------------------------------
			...
			function getFilterData(){
				...
				$.ajax({
					...
					url : "/cafe-bot/menu/filterproperties/readTypeConversion?fileid="+$("#property_name").val()
						...
						if (result != null && result.length > 0) {
							...
							$.each(result, function() {
								  $.each(this, function(k, v) {
									  ...
									  else if (k == "istimeseries"){
										if(v==1){
											$("#overwritediv").show();
											$("#namediv").show();
											$("#existing_collection").prop( "checked", true );
											name=0;
											if($(v==1)
												{
													$("#overwrite_collection").prop( "checked", true );
												}
												else 
												{
													$("#overwrite_collection").prop( "checked", false );
												}
										}
										else{
											$("#overwritediv").hide();
											$("#namediv").hide();
											$("#existing_collection").prop( "checked", false );
											name=1;
											$("#overwrite_collection").prop( "checked", false );
										}
							 }
		    ...
		    {		
				...
				if (index_type == "")
				{
					$("#btnDBExecute").notify("Please enter index type!","error");
					return;
				}
				var metric_type=$("#metric_type").val();
				if (metric_type == "")
				{
					$("#btnDBExecute").notify("Please enter metric type!","error");
					return;
				}
				...
				var existing_collection = 0;
				var overwrite_collection= 0;
				var name="";

				if($("#existing_collection").prop( "checked")==false)
				{
					existing_collection = 0;
					name="";
					if($("#overwrite_collection").prop( "checked")==false)
					{
						overwrite_collection = 0;
					}
					else 
					{
				...
				NProgress.start();
					$.ajax({
							method : "POST",
							url : "/cafe-bot/api/executeTypeConversion",
							data : {
			...
			function btnFilterClick() 
		       {	
				...
				NProgress.start();
					...
					$.ajax({
							method : "POST",
							url : "/cafe-bot/menu/filterproperties/updateTypeConversion",
							data : {
									fileid :	$("#property_name").val(),
									componentid :	$("#shape_id").val(),
